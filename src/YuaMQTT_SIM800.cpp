/******************************************
* YuaMQTT_SIM800.cpp
* SIM800 AT command helper implementation.
* Kept minimal for Arduino Uno flash/RAM budget.
******************************************/

#include "YuaMQTT_SIM800.h"

YuaSIM800::YuaSIM800(Stream &simSerial, Stream *debugSerial)
    : _sim(simSerial), _dbg(debugSerial) {}

bool YuaSIM800::waitResponse(const char *expected, unsigned long timeout) {
...
bool YuaSIM800::sendATCommand(const char *cmd, const char *expected, unsigned long timeout) {
    if (_dbg) { _dbg->print("> "); _dbg->println(cmd); }
    _sim.println(cmd);
    return waitResponse(expected, timeout);
}

bool YuaSIM800::begin() {
    // Wake up module
    sendATCommand("AT");
    delay(500);
...
int YuaSIM800::signalQuality() {
    if (_dbg) { _dbg->println("> AT+CSQ"); }
    _sim.println("AT+CSQ");

    unsigned long start = millis();
    String response = "";
    while (millis() - start < SIM800_AT_TIMEOUT) {
        while (_sim.available()) {
            char c = _sim.read();
            response += c;
        }
...
bool YuaSIM800::gprsConnect(const char *apn, const char *user, const char *pass) {
    // Check GPRS attachment
    if (!sendATCommand("AT+CGATT=1")) return false;

    // Set APN - SIM800 uses AT+CSTT
    char cmd[80];
...
bool YuaSIM800::gprsDisconnect() {
    return sendATCommand("AT+CIPSHUT", "SHUT OK", 5000);
}

bool YuaSIM800::tcpConnect(const char *host, uint16_t port) {
    char cmd[80];
    snprintf(cmd, sizeof(cmd), "AT+CIPSTART=\"TCP\",\"%s\",\"%u\"", host, port);
    return sendATCommand(cmd, "CONNECT OK", SIM800_CIPSTART_TIMEOUT);
}

bool YuaSIM800::tcpClose() {
    return sendATCommand("AT+CIPCLOSE", "CLOSE OK", 5000);
}

bool YuaSIM800::tcpSend(const uint8_t *data, uint16_t len) {
    if (!data || len == 0) return false;

    if (_dbg) { _dbg->println("> AT+CIPSEND"); }
    _sim.println("AT+CIPSEND");
    delay(SIM800_CIPSEND_DELAY);
...
uint16_t YuaSIM800::tcpRead(uint8_t *buf, uint16_t buf_size, unsigned long timeout_ms) {
